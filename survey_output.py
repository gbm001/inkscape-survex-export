...
class SurveyOutput:
    ...
    def __init__(self, stream, survey_name='mainsurvey', filename=None):
...
class TherionOutput(SurveyOutput):
    ...
    def _footer(self):
        """File footer"""
        if self.filename:
            file_stem = Path(self.filename).stem
        else:
            file_stem = self.survey_name
            self._write_line(f'# input "{file_stem}.th2"')
        self._write_line()
        self._write_line(f'{self.END_SURVEY}{self.survey_name}')
...
class SurvexOutputExtension(inkex.extensions.OutputExtension):
    ...
    def save(self, stream):
        ...
        # If we made it this far we're ready to write the survex/therion file
        ...
        if self.options.therion:
            survey_outputter = TherionOutput(stream, toplevel, output)
        else:
            survey_outputter = SurvexOutput(stream, toplevel, output)
        ...
        output_format = "therion" if self.options.therion else "survex"
        comments = [f'{output_format} file autogenerated from {docname}']
